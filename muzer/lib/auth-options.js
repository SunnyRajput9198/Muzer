...
const auth_utils_1 = require("@/lib/auth-utils"); // ✅ Adjust the path if needed
const cridentials_schema_1 = require("../schema/cridentials-schema");
    ...
    providers: [
        ...
        (0, credentials_1.default)({
            ...
            async authorize(credentials) {
                ...
                const emailValidation = cridentials_schema_1.emailSchema.safeParse(credentials.email);
                if (!emailValidation.success) {
                    throw new Error("Invalid email");
                }
                // Validate password using Zod schema
                const passwordValidation = cridentials_schema_1.passwordSchema.safeParse(credentials.password);
                ...
                try {
                    ...
                    if (!user) {
                        const hashedPassword = await bcryptjs_1.default.hash(passwordValidation.data, 10);
                    ...
                    const passwordVerification = await bcryptjs_1.default.compare(passwordValidation.data, user.password);
    ...
    callbacks: {
        async jwt({ token, account, user }) {
            ...
            if (!token.appToken && token.id) {
                ...
                const appToken = (0, auth_utils_1.generateAppToken)({ userId: token.id, creatorId });
